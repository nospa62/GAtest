...
def objective_function(X, net, penalty, time_interval):
    ...
    # Generator limits and ramp rates
    for i, gen in net_copy.res_gen.iterrows():
        p_mw_current, q_mvar_current = gen.p_mw, gen.q_mvar
        p_mw_prev = net.prev_gen_p_mw[i]
        params = net.gen_adv_params[i]
        gen_table = net_copy.gen.iloc[i]

        # Min/Max P and Q
        if not (gen_table.min_p_mw <= p_mw_current <= gen_table.max_p_mw):
            total_penalty += penalty
        if not (gen_table.min_q_mvar <= q_mvar_current <= gen_table.max_q_mvar):
            total_penalty += penalty
    ...
    # Slack generator limits and ramp rates
    slack_p_mw_prev = net.prev_slack_p_mw
    slack_params = net.slack_adv_params
    slack_table = net_copy.ext_grid.iloc[0]

    # Min/Max P and Q for slack
    if not (slack_table.min_p_mw <= slack_p_mw <= slack_table.max_p_mw):
        total_penalty += penalty
    if not (slack_table.min_q_mvar <= slack_q_mvar <= slack_table.max_q_mvar):
        total_penalty += penalty

    # Ramp rate for slack
    max_ramp_up_slack = slack_params['ramp_up_mw_min'] * time_interval
    max_ramp_down_slack = slack_params['ramp_down_mw_min'] * time_interval
    if (slack_p_mw - slack_p_mw_prev > max_ramp_up_slack) or \
            (slack_p_mw_prev - slack_p_mw > max_ramp_down_slack):
        total_penalty += penalty
